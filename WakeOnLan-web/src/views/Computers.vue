<template>
      <v-container>
        <v-row class="my-5">
          ...
          >
          <!--Añadir para que al hacer click en uno pase al estado de loading y si alguien ya lo ha arrancado aparezca disabled-->
            <v-card :to="'/'" hover exact height="200">
              <v-row>
                <v-col cols="4" sm="2" md="2" xl="3">
                  <v-icon class="display-4 display-none">mdi-desktop-classic</v-icon>
                </v-col>
                <v-col>
                  <v-card-title>{{computer.name}}</v-card-title>
                  <v-card-subtitle>{{computer.ip}}</v-card-subtitle>
                </v-col>
              </v-row>
            </v-card>
          </v-col>
